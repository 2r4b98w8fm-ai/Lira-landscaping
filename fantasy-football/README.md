...
| Live scoreboard (Scoreboard page) | ESPN's `mMatchup` `totalPoints` for the current week, the same field already synced for the schedule | **Real**, updates as it comes off the auto-sync every 60s — a blank score means that side hasn't posted points yet, not that it's actually 0 |
| FAAB (waiver budget) total + suggested bid | ESPN league settings (`acquisitionSettings.acquisitionBudget`) for the total, `teams[].transactionCounter.acquisitionBudgetSpent` for what you've spent | **Real** for the budget itself; the suggested bid $ is a documented heuristic (value-added scaled against your real remaining budget, capped at 40% on one add) — never presented as a guaranteed winning number, and the Waivers page simply omits it for a league that doesn't use FAAB |
| Player consistency (boom/bust vs. consistent floor, Start/Sit page) | Coefficient of variation (stdev/mean) of a player's own real weekly fantasy points this season, from nflverse's game log | **Real**, requires 3+ games played or it's omitted rather than guessed |
